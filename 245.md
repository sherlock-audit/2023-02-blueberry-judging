...
Now Alice invoke [closePosition()](https://github.com/sherlock-audit/2023-02-blueberry/blob/main/contracts/spell/IchiVaultSpell.sol#L341-L365) to close only 50%
with the following parameters  

```solidity
lpTakeAmt = type(uint256).max;
amountRepay = 2500e6;//2500 ==> only 50%
amountLpWithdraw = 2500e6;//2500 ==> only 50%
amountShareWithdraw = 0;
```
These are the critical values of Alice's position
getPositionValue Is: 0 USD
getDebtValue Is: 2500 USD
getUnderlyingValue Is: 14925 USD

Also, SPELL still has **2500e6** of **ICHI Vault LP** tokens on it.

These tokens can anyone gain them easily.  

Alice will lose 50% in this case (this depending on the values of `amountLpWithdraw` and `amountRepay`) 

## Impact
Users could leave some of `ICHI Vault LP` tokens on SPELL by mistakenly 
